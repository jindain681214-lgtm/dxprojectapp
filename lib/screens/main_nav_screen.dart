...
class _MainNavScreenState extends State<MainNavScreen> {
  ...
  String _currentScenarioKey = 'vn_standard'; // 1. 기본 시나리오 키 변경
  ...
  List<Device> removedDevices = []; // 제거된 기기 초기 목록은 비움

  void _removeDevice(Device device) {
    setState(() {
      myDevices.removeWhere((d) => d.id == device.id);
      if (!removedDevices.any((d) => d.id == device.id)) {
  ...
  void _addDevice(Device device) {
    setState(() {
      removedDevices.removeWhere((d) => d.id == device.id);
      if (!myDevices.any((d) => d.id == device.id)) {
  ...
  void _changeScenario(String key) {
    ...
    String statusMsg = "";
    if (key != 'vn_standard') { // 표준이 아닐 경우에만 알림
      ...
      statusMsg = "수질 상태가 안정적으로 복원되었습니다.";
  ...
  Widget build(BuildContext context) {
    // 2. 기본값 키 변경
    ScenarioData currentData = scenarioData[_currentScenarioKey] ?? scenarioData['vn_standard']!;
    ...
    final List<Widget> pages = [
      ...
      WaterQScreen(
        ...
        onNavigateToWeather: () {
          Navigator.push(context, MaterialPageRoute(builder: (context) => WeatherTestScreen(
              devices: myDevices,
              scenarioData: currentData,
          )));
        },
      ...
      endDrawer: Drawer(
        child: Column(
          children: [
            ...
            // 3. 새로운 시나리오 버튼들로 UI 업데이트
            Expanded(
              child: ListView(
                padding: const EdgeInsets.all(20),
                children: [
                  _buildGroupTitle("✅ 표준 (Standard)"),
                  _buildScenarioBtn("1. 표준 수질", 'vn_standard', Colors.blue),
                  const SizedBox(height: 30),
                  
                  _buildGroupTitle("⚠️ 탁도 (Turbidity)"),
                  _buildScenarioBtn("2. 탁도 주의 (Caution)", 'vn_turbidity_caution', Colors.orange),
                  _buildScenarioBtn("3. 탁도 위험 (Danger)", 'vn_turbidity_danger', Colors.red),
                  const SizedBox(height: 30),

                  _buildGroupTitle("⚡️ 전도도 (Conductivity)"),
                  _buildScenarioBtn("4. 전도도 주의 (Caution)", 'vn_conductivity_caution', Colors.orange),
                  _buildScenarioBtn("5. 전도도 위험 (Danger)", 'vn_conductivity_danger', Colors.red),
                  const SizedBox(height: 30),

                  _buildGroupTitle("🧪 철분 (Iron)"),
                  _buildScenarioBtn("6. 철분 위험 (Danger)", 'vn_iron_danger', Colors.deepOrange),
                ],
              ),
            ),
          ],
  ...
  Widget _buildScenarioBtn(String title, String key, Color dotColor) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isAvailable ? (isActive ? dotColor.withOpacity(0.1) : Colors.white) : Colors.grey[100],
        ...
        child: Row(
          children: [
            Container(
              width: 10, height: 10,
              decoration: BoxDecoration(shape: BoxShape.circle, color: isAvailable ? dotColor : Colors.grey),
            ),
